...
import { managerSnapshot } from "../../lib/assignments.mjs";
...
export async function GET(request: Request) {
  ...
  if (!decision.allowed) return Response.json({ error: "Manager dashboard requires the view-manager-dashboard capability.", reason: decision.reason }, { status: 403 });
  ...
  for (const user of users) {
    memberships[user.userId] = await store.listUserCohortIds(organisationId, user.userId);
  }
