...
export async function POST(request: Request) {
  if (request.headers.get("x-demo-role") !== "vendor-administrator" || request.headers.get("x-organisation-id") !== "org-nexus") return Response.json({ error: "Administrator access required." }, { status: 403 });
  ...
  const key = `org-nexus/${new Date().toISOString().slice(0, 10)}/${id}-${safeName}`;
  await runtime.SOURCES.put(key, file.stream(), { httpMetadata: { contentType: file.type }, customMetadata: { organisationId: "org-nexus", sourceId: id } });
  console.log(JSON.stringify({ event: "source_uploaded", organisationId: "org-nexus", sourceId: id, contentType: file.type, size: file.size, timestamp: new Date().toISOString() }));
